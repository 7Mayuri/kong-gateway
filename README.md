...
The script prints a clickable `file://` link when it finishes:

```text
  HTML report
    file  D:\...\kong-poc\test-report.html
    link  file:///D:/.../kong-poc/test-report.html
```

Ctrl+click the link, or have the script open it for you:
...
Note that `docker-compose up` alone does not produce the report; it only starts the
containers. The report is generated by the test script, which you run afterwards against
the running stack.
